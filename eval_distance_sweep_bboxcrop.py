"""train_bboxcrop.py で学習したモデルに対する距離スイープ評価。

eval_distance_sweep.py が「画像全体をそのままリサイズする」現行モデル
(distance=10でしか精度が出ない)を評価したのに対し、これは
bboxcrop_dataset.py と同じ bbox クロップ前処理(train_bboxcrop.pyが
学習時に使ったのと同一の padding_factor)を使って、bboxクロップ版モデルが
距離によらず精度を保てているかを確認する。

チェックポイントに保存された padding_factor (train_bboxcrop.py が保存)を
そのまま使うので、学習時と評価時で前処理がズレることはない。

matlab/generate_distance_sweep_database.m が出力する
`image,qw,qx,qy,qz,distance` 形式の labels.csv (既存の database_distance_sweep)
をそのまま再利用でき、MATLABの再レンダリングは不要。

Colab での使用例:
    !python eval_distance_sweep_bboxcrop.py \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default="distance_sweep_bboxcrop_output")
...
def main() -> None:
    ...
    summary_path = out_dir / "distance_sweep_bboxcrop_summary.csv"
    ...
    ax.set_title("bboxクロップモデル: 距離変化に対する姿勢推定精度")
    ax.legend()
    fig.tight_layout()
    plot_path = out_dir / "distance_sweep_bboxcrop_error.png"
    ...
    ax.set_xlabel("レンダリング距離")
    ax.set_ylabel("対称性考慮角度誤差 [deg]")
    ax.set_title("bboxクロップモデル: 距離ごとの誤差分布")
    ax.set_yscale("log")
    fig.tight_layout()
    box_path = out_dir / "distance_sweep_bboxcrop_boxplot.png"
    fig.savefig(box_path, dpi=120)
    plt.close(fig)
    print(f"保存: {box_path}")

    print(f"\n完了。結果は {out_dir}/ に保存されました。"
          f"\neval_distance_sweep.py (bboxクロップ無し) の結果と見比べてください。")
